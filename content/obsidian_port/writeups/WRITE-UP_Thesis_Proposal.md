...
We assume that one's emotional policy can only be lossily inferred by others; specifically, one's current "real" conscientiousness ($C_t$) is noisily encoded by others as $C_t'$ according to the function $C_t' = C_{t-s} + \epsilon, \epsilon \sim N(0,\sigma_C)$, where $s$ is some delay.
