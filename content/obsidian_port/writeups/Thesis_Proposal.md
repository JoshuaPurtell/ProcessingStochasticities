...
While there is a genetic component to $C$ (apparently anti-correlated [2] with $g$, as one would expect given Berkson's Law), there is a significant environmental factor (around 50% [3]). Furthermore, conscientiousness changes over an individual's lifetime, often for the better [4].
...
We assume that one's emotional policy can only be lossily inferred by others; specifically, one's current "real" conscientiousness ($C_t$) is noisily encoded by others as $C_t'$ according to the function $C_t' = C_{t-s} + \epsilon, \epsilon \sim N(0,\sigma_C)$, where $s$ is some delay factor. 
...
(4) https://www.apa.org/monitor/julaug03/personality
 ...
 <iframe seamless src="/obsidian_port/nodes/Thesis_Proposal.html" style="width:700px; height:400px; border: 2px solid black"></iframe>